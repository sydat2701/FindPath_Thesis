...
class BFS {
    private:
        ...
        vector<vector<int> >selected_slots;
    public:
        BFS(int _n, int _m, vector<vector<int> > _grid, vector<vector<int> > _slots, vector<vector<int> > _selected_slots) : n(_n), m(_m){
            grid = _grid;
            slots = _slots;
        ...
        bool check_and_lock(int car_id, int x, int y) {
            if(selected_slots[car_id][0] == -1) {
                lock(x, y);
                return true;
            }
            if((x >= selected_slots[car_id][0]) && (x <= selected_slots[car_id][2]) && (y >= selected_slots[car_id][1]) && (y <= selected_slots[car_id][3])) {
                for(int i = selected_slots[car_id][0]; i <= selected_slots[car_id][2]; i++)
                    for(int j = selected_slots[car_id][1]; j <= selected_slots[car_id][3]; j++)
                        grid[i][j] = BLOCKED;
                return true;
            }
            return false;
                ...
                while(!q.empty()) {
                    ...
                    for(int i = 0; i < NDIR; i++) {
                        ...
                        if(grid[to_2D_x][to_2D_y] == TARGET) {
                            // printf("target = %d %d\n", to_2D_x, to_2D_y);
                            if(check_and_lock(isource, to_2D_x, to_2D_y)) {
                                oops_2D_x = to_2D_x;
                                oops_2D_y = to_2D_y;
...
PYBIND11_MODULE(FindPathCplusplus, handle) {
    ...
    pybind11::class_<BFS>(handle, "BFS")
        .def(pybind11::init<int, int, vector<vector<int>>, vector<vector<int>>, vector<vector<int>> >())
